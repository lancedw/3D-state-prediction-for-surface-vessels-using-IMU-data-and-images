...
from constants import LEN_SEQ, SEQ_PER_EPISODE_C,  RANDS, RES_DIR
from PIL import Image


"""
This function prepares the results of testing in format which is writable in *.json file
"""
...
"""
Load index to get right data from DataLoader
"""
def loadLabels(folder_prefix, N_episodes_st, N_episodes_end, seq_per_ep, p_train=0.7, p_val=0.15, p_test=0.15):
    """
    Args:
        folder_prefix (str):
        N_episodes_st (int): first episode number which is used for experiment
        N_episodes_end (int): last episode number which is used for experiment
        seq_per_ep (int): number of sequences in an episode
    ...
    all_ep = np.linspace(
        N_episodes_st*seq_per_ep, 
        N_episodes_end*seq_per_ep, 
        (N_episodes_end*seq_per_ep - N_episodes_st*seq_per_ep + 1), 
        dtype =int 
    )
...
class JsonDataset_universal (Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(tmp_use_n_im):
            ...
            image_nr = (seq_in_episodes-1)*tmp_use_n_im + i
            image_str = str(image_nr) + ".png"
            ...
            # get PR from labels file that corresponds to the image
            tmp_label = np.array(labels_episode[str(image_nr)])

            # normalize pixels and PR
            if self.preprocess:
        ...
        if not self.use_LSTM:
            for i in range(self.predict_n_im):
                ...
                if self.change_fps:
                    if i%2 != 0:
                       continue
                image_nr = (seq_in_episodes-1)*tmp_use_n_im + tmp_use_n_im + i
                tmp_label_future = np.array(labels_episode[str(image_nr)])
